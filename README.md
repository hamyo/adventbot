...
## Сборка
Приложение написано на java 24. Использует СУБД Postgres. Для сборки необходимо jdk 24 версии (Например, [OpenJDK](https://jdk.java.net/24)). Для развертывания желательно иметь docker, но это не обязательно.

### Сборка jar приложения
После клона проекта нужно собрать jar с помощью gradle. 
